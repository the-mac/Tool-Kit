namespace MyCompany.Common.Security.Web
{
    ...
    public class SecurityHelper : ISecurityHelper
    {
        ...
        static string audience = ConfigurationManager.AppSettings["AllowedAudience"];
        ...
        /// <summary>
        /// <see cref="MyCompany.Common.Security.Web.ISecurityHelper"/>
        /// </summary>
        /// <returns><see cref="MyCompany.Common.Security.Web.ISecurityHelper"/></returns>
        public string GetUser()
        {
            var principal = (ClaimsPrincipal)Thread.CurrentPrincipal;

            // Test Mode is only for demos without internet, not for real apps!
            if (RequestIsNoAuthRoute() || !principal.Identity.IsAuthenticated)
                return System.Configuration.ConfigurationManager.AppSettings["testModeIdentity"];

            var nameClaim = principal.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name || c.Type == ClaimTypes.Upn);
