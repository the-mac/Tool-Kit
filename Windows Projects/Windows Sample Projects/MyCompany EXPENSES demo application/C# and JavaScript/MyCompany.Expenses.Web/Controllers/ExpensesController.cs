...
namespace MyCompany.Expenses.Web.Controllers
{
    ...
    /// <summary>
    /// Expenses Controller
    /// NOTE: this controller is not authorized for demo pourposes.
    /// </summary>   
    ...
    public class ExpensesController : ApiController
    {
        ...
        [Route("~/noauth/api/expenses/{expenseId:int:min(1)}/{pictureType:int:range(1,2)}")]
        ...
        [Route("user")]
        [Route("~/noauth/api/expenses/user")]
        ...
        [WebApiOutputCacheAttribute(true)]
        [Route("team")]
        [Route("~/noauth/api/expenses/team")]
        ...
        [Route("~/noauth/api/expenses/user/{expenseStatus:int:min(0)}/count")]
        ...
        [Route("~/noauth/api/expenses/team/{expenseStatus:int:min(0)}/count")]
        ...
        [Route("~/noauth/api/expenses/teammembers/{pictureType:int:range(1,2)}")]
        ...
        [Route("~/noauth/api/expenses/teammembers/count")]
        ...
        [Route("~/noauth/api/expenses/team/month/{expenseType:int:min(0)}")]
        ...
        [Route("~/noauth/api/expenses/teammember/{employeeId:int:min(1)}/month/{expenseType:int:min(0)}")]
        ...
        [Route("~/noauth/api/expenses/teammember/{employeeId:int:min(1)}/summary/{year:int}/{month?}")]
        ...
        private async Task NotifyToManagers(int expenseId)
        {
            try
            {
                var expense = await _expenseRepository.GetAsync(expenseId);
                ...
                {
                    _notificationService.NewExpenseAdded(notificationChannel, expense);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task NotifyToEmployeeToWindowsPhone(Expense expense)
        {
            try
            {
                var userChannels = await _notificationChannelRepository.GetUserChannelsAsync(expense.Employee.Email);

                foreach (var notificationChannel in userChannels)
                {
                    if (notificationChannel.NotificationType == NotificationType.WindowsPhoneNotification)
                        _notificationService.ExpenseStatusChanged(notificationChannel, expense);
                }
            }
            catch (Exception ex)
            {
